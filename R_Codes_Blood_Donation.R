...
pollutantmean <- function(directory, pollutant, id= 1:332) { 
  setwd("C:/Users/Rachana/Documents/Rutgers-MBS/Rcoursera")
 ...
 for (i in id)
    { 
      d<- read.csv(fi[i])
     sum=sum+colMeans(d[pollutant],na.rm=TRUE)
     count =count+1
    }
 mean= sum/count
 return  (mean)
...
pollutantmean <- function(directory, pollutant, id= 1:332) { 
  ...
  { 
    d<- read.csv(fi[i])
    sum=sum+colSums(d[pollutant],na.rm=TRUE)
    count =count+sum(!is.na(d[pollutant]))
  }
  mean= sum/count
  return (mean)
}
